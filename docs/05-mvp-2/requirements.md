...
**Date:** 2026-08-09  
**Source of Truth:** [claude-first-principle.md](file:///home/zackchow/coding/rckg/docs/04-deepdive/claude-first-principle.md)  
**Prior MVP Plan:** [mvp-sprint.md](file:///home/zackchow/coding/rckg/docs/03-mvp/mvp-sprint.md) (Sprint 1–4, 140 pts, largely incomplete against BRD)  
**Prior Assessment:** [claude-mvp-assessment.md](file:///home/zackchow/coding/rckg/docs/04-deepdive/claude-mvp-assessment.md) (8 genuinely fixed, 10 partially fixed, 5 not fixed)
...
| C-10 | **Soft-delete only (no permanent deletion)** | REQ-022 | PARTIAL | SUPERSEDE/DEPRECATE patterns exist. `delete_file()`, `delete_collection()` exist unguarded. [rckg_queries.py:L204-213](file:///home/zackchow/coding/rckg/backend/app/graph/rckg_queries.py#L204-L213), [storage/__init__.py:L242-253](file:///home/zackchow/coding/rckg/backend/app/storage/__init__.py#L242-L253) | **Must** | BRD §6 constraint. Guard delete methods against compliance data. | BR-14 |
| C-11 | **Open-source models only, no external API guard** | REQ-033 | PARTIAL | Mistral 8B + Llama 3.1. No hard guard preventing `LLM_ENDPOINT` pointing to OpenAI. [extraction.py:L39](file:///home/zackchow/coding/rckg/backend/app/services/extraction.py#L39) | **Must** | BRD §6 constraint. Add startup validation that `LLM_ENDPOINT` is local. | BRD §6 |
| C-12 | **Facet extractor with real AI** | (supports REQ-012) | NOT MET (Finding #3 in claude-mvp-assessment) | 10 hardcoded verbs, 10 nouns, 3 domains, `control_nature` always `PREVENTATIVE`. LLM try/catch added but regex fallback unchanged. [facet_extractor.py:L16-97](file:///home/zackchow/coding/rckg/backend/app/services/facet_extractor.py#L16-L97) | **Should** | Improves mapping quality significantly. Not strictly blocking — graph compiler can work with LLM extraction output directly. | BR-06 |
| C-13 | **ColBERT with real model** | REQ-016 | PARTIAL | MaxSim architecture correct. Uses synthetic hash-seeded embeddings, not ColBERTv2 model. [colbert_service.py:L33-50](file:///home/zackchow/coding/rckg/backend/app/services/retrieval/colbert_service.py#L33-L50). FINDING-008. | **Should** | Improves retrieval accuracy. Bi-encoder or BM25 can serve as interim. | BR-08 |
| C-14 | **Bitemporal fields on all node types + edges** | REQ-018 | PARTIAL | 2 of 7 node types have `valid_from`/`valid_to`. 0 edge tables have bitemporal. [rckg_nodes.py:L89-92](file:///home/zackchow/coding/rckg/backend/app/models/rckg_nodes.py#L89-L92). FINDING-005. | **Should** | Data model correctness. Not blocking core flow but blocking temporal queries. Add columns in migration. | BR-10 |
...
| C-16 | **Temporal.io concrete workflow (ingestion pipeline)** | REQ-028 | PARTIAL | `BaseWorkflow` has pause/approve signals. `execute()` raises `NotImplementedError`. [workflows/base.py:L30-116](file:///home/zackchow/coding/rckg/backend/app/workflows/base.py#L30-L116) | **Could** | Direct service calls work for MVP. Temporal adds resilience but not core value. | BR-20 |
...
_Traces to: C-01 | Current state: Exists ([document_upload.py:L54-215](file:///home/zackchow/coding/rckg/backend/app/services/document_upload.py#L54-L215)) | Work: None — already functional._
...
_Traces to: C-01 | Current state: Stubbed ([pdf_to_markdown.py:L157-182](file:///home/zackchow/coding/rckg/backend/app/services/pdf_to_markdown.py#L157-L182)) | Work: Implement real MinerU/Marker calls — replace `RuntimeError` stubs with actual library integration._
...
_Traces to: C-02 | Current state: Exists ([hybrid_chunking.py](file:///home/zackchow/coding/rckg/backend/app/services/hybrid_chunking.py)) | Work: Verify against real MinerU output._
...
_Traces to: C-03 | Current state: Partially functional ([extraction.py:L283-326](file:///home/zackchow/coding/rckg/backend/app/services/extraction.py#L283-L326)) | Work: Harden error handling. Ensure returned items have all 6 fields populated._
...
_Traces to: C-03 | Current state: Exists ([extraction.py:L46-51](file:///home/zackchow/coding/rckg/backend/app/services/extraction.py#L46-L51)) | Work: None — already functional._
...
_Traces to: C-04, C-06 | Current state: LLM path exists ([nli_engine.py:L62-76](file:///home/zackchow/coding/rckg/backend/app/services/nli_engine.py#L62-L76)); keyword fallback produces fake confidence | Work: Remove keyword fallback or cap its confidence at 0.30 with `extraction_method: "HEURISTIC_DEGRADED"`._
...
_Traces to: C-04, U-04 | Current state: Keyword fallback produces 0.88-0.95 confidence ([nli_engine.py:L85-180](file:///home/zackchow/coding/rckg/backend/app/services/nli_engine.py#L85-L180)) | Work: Replace keyword fallback with explicit failure/pending state._

**FR-10** — Graph compiler shall create `Gap` nodes for both `NO_RELATIONSHIP` (cosine < 0.30) AND `SUBSET_OF` classifications, per BR-07.  
_Traces to: C-06, C-07 | Current state: Gaps only for cosine < 0.30 ([graph_compiler.py:L75-88](file:///home/zackchow/coding/rckg/backend/app/services/graph_compiler.py#L75-L88)) | Work: Add gap creation rule for SUBSET_OF._
...
_Traces to: C-05 | Current state: Judge is async and disconnected from commit path ([memgraph_service.py:L115-173](file:///home/zackchow/coding/rckg/backend/app/services/memgraph_service.py#L115-L173)) | Work: Wire `DualJudgeService.evaluate()` into `enqueue_and_execute()` before Cypher execution._
...
_Traces to: C-05 | Current state: Thresholds are 0.80 and 0.85 ([judge.py:L44](file:///home/zackchow/coding/rckg/backend/app/services/judge.py#L44), [dual_judge_async.py:L84](file:///home/zackchow/coding/rckg/backend/app/services/dual_judge_async.py#L84)) | Work: Update threshold constants. Add `PENDING_HITL_REVIEW` status._
...
_Traces to: C-05, U-05 | Current state: `logic_score = confidence * 1.02` ([dual_judge_async.py:L82-85](file:///home/zackchow/coding/rckg/backend/app/services/dual_judge_async.py#L82-L85)) | Work: Replace arithmetic fallback with explicit pending state._
...
_Traces to: C-09 | Current state: Exists ([memgraph_service.py:L115-173](file:///home/zackchow/coding/rckg/backend/app/services/memgraph_service.py#L115-L173)) | Work: None — already functional. Wire judge gate in front._
...
_Traces to: C-10 | Current state: Unguarded delete methods ([storage/__init__.py:L242-253](file:///home/zackchow/coding/rckg/backend/app/storage/__init__.py#L242-L253)) | Work: Add bucket/collection guards._
...
_Traces to: C-11 | Current state: No validation ([extraction.py:L37](file:///home/zackchow/coding/rckg/backend/app/services/extraction.py#L37)) | Work: Add startup check in app initialization._
...
**NFR-03 — Observability:** All degradation events (LLM failure, NLI fallback, judge unavailability) shall be logged via `log_degradation_event()` with `service`, `method_used`, `error`, and `context` fields. _Traces to: Existing pattern in [nli_engine.py:L78-84](file:///home/zackchow/coding/rckg/backend/app/services/nli_engine.py#L78-L84). Already partially implemented._

**NFR-04 — Data Integrity:** No SQL/Cypher injection possible via user-supplied document content. All Cypher queries shall use parameterized templates (already the case in [rckg_queries.py](file:///home/zackchow/coding/rckg/backend/app/graph/rckg_queries.py)). All MinIO keys shall be validated against path traversal ([bronze_layer.py:L22](file:///home/zackchow/coding/rckg/backend/app/services/bronze_layer.py#L22) pattern already exists).
...
| D-03 | Memgraph container accessible and schema initialized | Internal infra | Available ([docker-compose.yml:L60-82](file:///home/zackchow/coding/rckg/docker-compose.yml#L60-L82)) | Already containerized. |
| D-04 | PostgreSQL with schema migrations applied | Internal infra | Available ([docker-compose.yml:L30-55](file:///home/zackchow/coding/rckg/docker-compose.yml#L30-L55)) | Already containerized. |
